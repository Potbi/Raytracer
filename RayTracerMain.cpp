...
class TUser : public TPlan {
    ...
    TVektor berechneFarbe(Szene szene, Strahl s, int iteration){
        ...
        // ### LAMBERTIAN SHADING ###
            ...
            // Lichtquellen suchen
            for(int i=0; i<szene.anzObjekte; i++){
                float em = szene.objekte[i]->material.emission;
                std::cout<<em<<"\n";
    ...
    void Init(){
        ...
        const int XAUFL = 720/3;
        const int YAUFL = 480/3;
        const float BRENN = 4;
        ...
        */
        ...
        //szene->DreieckHinzufuegen(TVektor(7,7,7),TVektor(7,6.5,7),TVektor(7,7.5,7),mtl_leuchte);

        Material mtl_haut(RGBSkala(100,59,33), 0, 0);
        Material mtl_dunkelhaut(RGBSkala(81,48,27), 0, 0);
        Material mtl_rot(Rot, 0.01, 0);
        Material mtl_dunkelrot(RGBSkala(33,0,0), 0, 0);

        //szene->kugelHinzufuegen(TVektor(10,0,0), mtl_leuchte, 0.1);
        szene->DreieckHinzufuegen(TVektor(10,0,0),TVektor(10,0.1,0),TVektor(10,-0.1,0.1), mtl_leuchte);
        szene->kugelHinzufuegen(TVektor(0,0,0), mtl_rot, 3);

        Kugel k(TVektor(10,0,0), mtl_leuchte, 0.1);
